...
export class LocationThreePage {
  ...
  loadIdeas() {
    return this.getUserInfo().then( (location_three) => {
      console.log("List Ideas Location ID: ", location_three);
      return this.databaseprovider.getAllIdeas(location_three).then(data => {
  ...
  async updateContent() {
    return await this.storage.get('location_three').then( (location_three) => {
      return this.databaseprovider.getLocationById(parseInt(location_three)).then( (location) => {
